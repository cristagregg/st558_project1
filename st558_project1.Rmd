...
```{r get stats}
get_team_stats <- function(id = 'All'){
...
URL_combine <- paste0('https://statsapi.web.nhl.com/api/v1/teams/', as.character(get_endpoint_all$teams$id), '/stats', '?expand=team.stats')
...
return(full_join(team_stats_combined_games, team_stats_combined_wins, by = 'team.id'))

} else {
#if specific team wanted
  URL <- paste0('https://statsapi.web.nhl.com/api/v1/teams/', as.character(id), '/stats', '?expand=team.stats')
  get_endpoint <- GET(URL) %>%
      content("text", encoding = 'UTF-8') %>%
      fromJSON(flatten = T) 
  ...
  wins <- get_endpoint$stats$splits[[2]] %>% 
    select(team.id, team.name, everything())
  return(full_join(games, wins, by = 'team.id'))
}
}
